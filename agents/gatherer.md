...
After appending, if today.md exceeds 200 lines, drop the oldest section (from the first `## ` heading to just before the next `## ` heading) until the file is under 200 lines. Never drop the `# Daily Context` header line.
