...
2. Append an `## Activity [HH:MM]` entry to today.md:

```bash
TODAY=$(date +%Y-%m-%d)
TODAYMD="$KVIDO_HOME/memory/today.md"
# Reset file if new day
...
## Activity [HH:MM]
- <concise summary of what the user is working on or has just done>
EOF
```

Replace `HH:MM` with `$(date +%H:%M)`. Keep the bullet to one line — actionable and specific.
