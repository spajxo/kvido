...
4. Lightweight check:
   ...
   ```
   If gitlab source is enabled (`kvido config "sources.gitlab.enabled" "true"` returns `"true"`), also run `bash "$CLAUDE_PLUGIN_ROOT/scripts/fetch-gitlab-mrs.sh"` and grep for `<repo_name>`.
