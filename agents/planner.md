...
Before making any scheduling decisions, read `$KVIDO_HOME/memory/today.md` using the Read tool if it exists and has today's date header. It provides live context from gatherer, chat, and other agents — use it to inform prioritization without re-querying sources.

### Consolidate and clean today.md

At the start of each planner run, check whether today.md needs cleanup:
- If the file's header date is not today, replace the entire file with a fresh `# Daily Context — YYYY-MM-DD` header and a brief comment (`<!-- Auto-generated by kvido agents. Reset daily. Max ~200 lines. -->`). Use the Write tool directly — no shell scripts.
- If the file exceeds 200 lines, drop the oldest section (from the first `## ` heading to just before the next `## ` heading) until it is under 200 lines. Use the Edit tool.

This is a natural part of the planner's housekeeping role — no bash scripting needed.
