...
while IFS=$'\t' read -r repo_name repo_path repo_priority; do
  process_repo "$repo_name" "$repo_path" "${repo_priority:-normal}"
done < <($CONFIG '.sources.gitlab.repos[] | [.name, .path, .priority // "normal"] | @tsv')
