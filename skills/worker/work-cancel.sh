...
STATE=$(glab issue view "$ISSUE" --repo "$REPO" --output json | jq -r '.state')
if [[ "$STATE" != "opened" ]]; then
  echo "Error: issue #$ISSUE is not open (state=$STATE)" >&2
  exit 1
fi

glab issue note "$ISSUE" --repo "$REPO" --message "## Cancelled
...
if [[ -n "$REMOVE_LABELS" ]]; then
  glab issue update "$ISSUE" --repo "$REPO" --unlabel "$REMOVE_LABELS" --label "result:cancelled" >/dev/null
else
  glab issue update "$ISSUE" --repo "$REPO" --label "result:cancelled" >/dev/null
fi
